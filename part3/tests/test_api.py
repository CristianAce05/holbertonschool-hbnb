...
class HBNBApiTestCase(unittest.TestCase):
    ...
    def test_place_validation_and_relations(self):
        ...
        r = self.post_json(
            "/api/v1/users", {"email": "owner@example.com", "password": "pw"}
        )
        owner = r.get_json()
        uid = owner["id"]
    ...
    def test_review_crud_and_place_includes_reviews(self):
        ...
        r = self.post_json(
            "/api/v1/users", {"email": "ru@example.com", "password": "pw"}
        )
        uid = r.get_json()["id"]
        ...
        # create review
        r = self.post_json(
            "/api/v1/reviews", {"user_id": uid, "place_id": pid, "text": "Nice"}
        )
        self.assertEqual(r.status_code, 201)
        rev = r.get_json()
        rid = rev["id"]
        ...
        self.assertEqual(r.status_code, 200)
        place = r.get_json()
        self.assertTrue(any(x["id"] == rid for x in place.get("reviews", [])))
        # update review
