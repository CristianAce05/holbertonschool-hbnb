...
cur.execute(
    "INSERT INTO places (id, name, description, number_rooms, number_bathrooms, max_guest, price_by_night, latitude, longitude, user_id, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))",
    ('place-1', 'Admin House', 'Nice place', 3, 2, 5, 100, 40.0, -74.0, 'user-admin-1'),
)
conn.commit()
...
cur.execute("INSERT INTO place_amenity (place_id, amenity_id) VALUES (?,?)", ('place-1', 'amenity-1'))
conn.commit()
...
cur.execute(
    "INSERT INTO reviews (id, user_id, place_id, text, created_at, updated_at) VALUES (?,?,?,?,datetime('now'),datetime('now'))",
